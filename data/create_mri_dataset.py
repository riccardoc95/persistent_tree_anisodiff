...
class H5_Loader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # return gt_ksp
        # --- New Part: reconstruct image ---
        img = ifft2c(gt_ksp)  # 2D centered inverse FFT (you may need to define this function)
        img = np.abs(img)
        img = np.sqrt(np.sum(img ** 2, axis=0))  # Combines along coil/channel dimension
        img = img / np.max(img)
        return img.astype(np.float32)


def create_train(output_clean, output_corrupt, R=4, sigma=0.01):
    file_list = glob.glob(os.path.join("singlecoil_test", "*.h5"))
    input_dir = "singlecoil_test/"
    ...
    for images in tqdm(train_loader):
        ...
        for image, image_corrupt, mask in zip(images.unbind(0), images_corrupt.unbind(0), masks.unbind(0)):
            fits.writeto(os.path.join(output_clean, f"image_{indx:06d}.fits"), image.cpu().numpy(), overwrite=True)
            fits.writeto(os.path.join(output_corrupt, f"image_{indx:06d}.fits"), image_corrupt.cpu().numpy(),
                         overwrite=True)
...
output_dir = "mri_dataset"
os.makedirs(output_dir, exist_ok=True)

for sigma in [0.1, 0.2, 0.3]:
    create_train(os.path.join(output_dir, "true"), os.path.join(output_dir, f"image/s_{sigma}"), sigma=sigma)